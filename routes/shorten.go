...
func ShortenURL(c *fiber.Ctx) error {
	body := new(request)
	// автоматический парсер в request
	...
	if err == redis.Nil {
		...
		c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "invalid connect to redis"})
	} else {
		val, err = r2.Get(database.Ctx, c.IP()).Result()
	...
	if !helpers.RemoveDomainError(body.URL) {
		c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{"error": "Domain error"})
	}
